...
public class ModRecipeProvider extends RecipeProvider {
    ...
    protected void buildRecipes(RecipeOutput recipeOutput) {
        List<ItemLike> BISMUTH_SMELTABLES = List.of(ModItems.RAW_BISMUTH, ModBlocks.BISMUTH_BLOCK);
        ...
        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, ModBlocks.BISMUTH_BLOCK.get(), 9)
                .requires(ModBlocks.MAGIC_BLOCK)
                .unlockedBy("has_bismuth_block", has(ModBlocks.MAGIC_BLOCK))
                .save(recipeOutput, "subtleforces:bismuth_from_magic_block");
    ...
    protected static <T extends AbstractCookingRecipe> void oreCooking(RecipeOutput recipeOutput, RecipeSerializer<T> pCookingSerializer, AbstractCookingRecipe.Factory<T> factory,
        ...
        for(ItemLike itemlike : pIngredients) {
            SimpleCookingRecipeBuilder.generic(Ingredient.of(itemlike), pCategory, pResult, pExperience, pCookingTime, pCookingSerializer, factory).group(pGroup).unlockedBy(getHasName(itemlike), has(itemlike))
                    .save(recipeOutput, SubtleForces.MOD_ID + ":" + getItemName(pResult) + pRecipeName + "_" + getItemName(itemlike));
        }
    }
